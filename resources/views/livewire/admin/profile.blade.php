<div class="space-y-8 animate__animated animate__fadeIn max-w-4xl mx-auto">
    ...
    <!-- Page Header -->
    <div>
        <h2 class="text-3xl font-display font-bold text-gray-900 mb-1">Admin Profile</h2>
        <p class="text-sm text-gray-500">Manage your account information and security settings.</p>
    </div>
    ...
    <div class="glass-card rounded-[2.5rem] p-8 border border-gray-100 flex flex-col sm:flex-row items-center sm:items-start gap-8">
        ...
        <div class="shrink-0 flex flex-col items-center gap-3">
            ...
            <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-purple-100 text-purple-700 border border-purple-200">
                Administrator
            </span>
        </div>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                @php
                    $meta = [
                        ['label' => 'Role',     'value' => 'Administrator', 'icon' => 'shield-check'],
                        ['label' => 'Phone',    'value' => $user->phone ?? '—', 'icon' => 'phone'],
                        ['label' => 'Member Since', 'value' => $user->created_at->format('M Y'), 'icon' => 'calendar'],
                    ];
                @endphp
        ...
        <div class="flex items-center justify-between p-8 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">Update your display name and contact number</p>
                </div>
            </div>
            @if(!$editingProfile)
        ...
        <div class="flex items-center justify-between p-8 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">Keep your admin account secure with a strong password</p>
                </div>
            </div>
            @if(!$changingPassword)
        ...
        <div class="p-8">
            ...
            @else
                <p class="text-sm text-gray-400">
                    ...
                    <span class="font-semibold text-gray-600">{{ $user->updated_at->format('M d, Y') }}</span>
                </p>
            @endif
        </div>
    </div>
    ...
    <div class="glass-card rounded-[2.5rem] border border-gray-100 overflow-hidden">
        <div class="flex items-center gap-3 p-8 border-b border-gray-100 bg-gray-50">
            ...
            <div>
                <h4 class="text-base font-bold text-gray-900">System Access</h4>
                <p class="text-xs text-gray-400">Your administrative privileges</p>
            </div>
        </div>
        <div class="p-8 grid grid-cols-2 sm:grid-cols-4 gap-5">
            @php
                $privileges = [
                    ['label' => 'Exam Control',  'icon' => 'activity',        'color' => 'bg-blue-100 text-blue-600'],
                    ['label' => 'User Marks',    'icon' => 'award',           'color' => 'bg-emerald-100 text-emerald-600'],
                    ['label' => 'Certificates',  'icon' => 'scroll',          'color' => 'bg-amber-100 text-amber-600'],
                    ['label' => 'PDF Books',     'icon' => 'book',            'color' => 'bg-purple-100 text-purple-600'],
                ];
            @endphp
            @foreach($privileges as $p)
                <div class="p-4 rounded-xl bg-gray-50 border border-gray-100 flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg {{ $p['color'] }} flex items-center justify-center shrink-0">
