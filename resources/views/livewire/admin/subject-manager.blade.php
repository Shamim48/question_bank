<div class="space-y-8 animate__animated animate__fadeIn">
    @php
        $u = auth()->user();
        $canCreate = $u->isAdmin() || $u->hasPermission('subjects-create');
        $canEdit   = $u->isAdmin() || $u->hasPermission('subjects-edit');
        $canDelete = $u->isAdmin() || $u->hasPermission('subjects-delete');
    @endphp
    <!-- Header -->
    ...
    <div class="glass rounded-[2rem] overflow-hidden border border-white/5">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-white/5">
                    <th class="py-5 px-8 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Identification
                    </th>
                    <th class="py-5 px-8 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Round
                        Association</th>
                    ...
                    <tr class="group hover:bg-white/[0.02] transition-colors">
                        <td class="py-5 px-8">
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 group-hover:scale-110 transition-transform">
                        ...
                        <td class="py-5 px-8">
                            <span
                                class="inline-flex items-center gap-1.5 px-3 py-1 bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[10px] font-bold uppercase tracking-wider rounded-lg">
                                <i data-lucide="layers" class="w-3 h-3"></i> {{ $subject->round->name }}
                            </span>
                        </td>
                        <td class="py-5 px-8">
                            <div class="flex items-center gap-2">
                                <div class="flex -space-x-2">
                                    <div
                                        class="w-6 h-6 rounded-full border border-gray-900 bg-indigo-600 flex items-center justify-center text-[8px] font-bold text-white">
                        ...
                        <td class="py-5 px-8 text-right">
                            ...
                            <div class="flex items-center justify-end gap-2">
                                @if($canEdit)
                                <button wire:click="openForm({{ $subject->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="py-20 text-center">
                            <i data-lucide="database" class="w-12 h-12 text-gray-700 mx-auto mb-4"></i>
                            <p class="text-gray-500 font-medium">No subjects detected in system.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-2xl font-display font-bold text-gray-900">
                        {{ $editingId ? 'Refine Subject' : 'Initialize Subject' }}</h3>
